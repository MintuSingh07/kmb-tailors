...
export default async function ClientHistoryPage() {
  ...
  await dbConnect();
  const clients = await Client.find({}).select('-measurementDrawing -measurementDrawings -strokes -handoverImages -images').sort({ updatedAt: -1 });
      ...
      <header className="border-b border-slate-100 bg-white/80 backdrop-blur-xl px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between shadow-sm sticky top-0 z-40">
        <div className="flex items-center gap-3">
          ...
          <span className="font-extrabold text-lg sm:text-2xl tracking-tight text-[#1A1A1A]">
            KBM Boutique <span className="hidden min-[450px]:inline-block font-semibold text-slate-500 text-sm sm:text-lg ml-1.5 border-l border-slate-200 pl-2.5">Client History</span>
          </span>
        </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 py-10 w-full">
        ...
        {clients.length === 0 ? (
          <div className="rounded-3xl border border-[#E6DFD3] bg-[#FCFAF5] p-12 text-center shadow-xl shadow-slate-200/30 flex flex-col items-center justify-center max-w-2xl mx-auto">
            ...
            <p className="text-slate-500 font-semibold mb-6">No client records saved in KBM Boutique database yet.</p>
            <Link
              href="/admin/new"
