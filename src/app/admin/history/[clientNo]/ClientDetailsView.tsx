          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 sm:gap-10">
            {clientRecords.map((q) => {
              ...
              return (
                ...
                >
                  {/* Card Banner showing images: Fabric & Outfit (for completed/prepared) or Fabric, Outfit & Sketch (for pending) */}
                  {(() => {
                    const isCompletedOrPrepared = q.suitStatus === 'Completed and handovered' || q.suitStatus === 'Prepared but not handovered';
                    return (
                      <div className={`grid ${isCompletedOrPrepared ? 'grid-cols-2' : 'grid-cols-3'} h-72 sm:h-80 lg:h-96 bg-slate-50 border-b border-[#E6DFD3] select-none overflow-hidden relative`}>
                        ...
                        {completedImg ? (
                          <div
                            ...
                            className={`relative w-full h-full ${!isCompletedOrPrepared ? 'border-r border-[#E6DFD3]' : ''} overflow-hidden bg-slate-100 cursor-zoom-in group/outfit`}
                          >
                            <img
                              src={completedImg}
                          ...
                          <div className={`w-full h-full flex flex-col items-center justify-center bg-slate-50 ${!isCompletedOrPrepared ? 'border-r border-[#E6DFD3]' : ''} text-slate-400 shrink-0`}>
                            <svg className="h-6 w-6 sm:h-7 sm:w-7 text-slate-300 mb-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                        ...
                        {/* Column 3: Measurement Sketch Image (only for pending suits) */}
                        {!isCompletedOrPrepared && (
                          sketchImg ? (
                            <div
                              onClick={() => {
                  ...
                  <div className="p-8 sm:p-10 flex flex-col flex-1 justify-between gap-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 pt-5 border-t border-slate-100 select-none">
                      {q.suitStatus !== 'Completed and handovered' && q.suitStatus !== 'Prepared but not handovered' && (
                        <Link
